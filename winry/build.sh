...
make_base_winry() {
    cp -f ${script_path}/pacman.conf ${work_dir}/${arch}/airootfs/etc
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "winry-mirrorlist winry-system" install
}
